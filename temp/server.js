...
/* =========================
   FITNESS DATA (LAST 7 DAYS) - IST FIX
========================= */
app.get("/getFitnessData", async (req, res) => {
  if (!req.session.accessToken) return res.status(401).send("Not authenticated");

  try {
    // IST Correct start & end time
    const now = new Date();
    ...
    response.data.bucket.forEach(bucket => {
      ...
      // FIX DATE TO IST
      const dateObj = new Date(Number(bucket.startTimeMillis));
...
const prompt = `
You are TOM, an AI Fitness Coach.

User fitness data (last 7 days):
...
- Include 1 positive trend
- Include 1 risk or concern (if any)
- Ask exactly 1 clarification question about ACTIVITY INTENSITY
  (intense workouts vs normal daily movement)
- Give exactly 3 tips:
  • at least 1 PREVENTIVE health tip
  • at least 1 ACTIVITY-related tip
  • at least 1 RECOVERY or lifestyle tip
- No diagnosis
